...
<head>
    ...
    <script>
        $(function () {
                $.ajax({
                    url: '${pageContext.request.contextPath}/message/selectOfflineMessage.action?friendName=<%=request.getParameter("friendName")%>',
                    type: 'POST',
                    async: true,
                    dataType: 'JSON',
                    success: function (data) {
                        ...
                        $.each(json,function(index,item){
                           var friendName=json[index].friendName;
                            ...
                            $("#content").append('<div class="fmsg"><label class="name">'+friendName+'&nbsp;'+sendDaetString+
                                    '</label><div class="sfmsg_text">'+text+'</div>');
                            scrollToBottom();
    ...
    <script>
        ...
        var friendName='<%=request.getParameter("friendName")%>';
...
<body>
<tittle  style="text-align: center">欢迎:${userName} 发送给:<%=request.getParameter("friendName")%></tittle>
<div id="content"></div>
<input type="text" placeholder="请输入要发送的信息" id="msg" class="msg" onkeydown="send(event)">
<input type="button" style="text-align: center" value="发送" class="send" onclick="sendMsg()" >
<input type="button"  style="text-align: center" value="清空" class="clear" onclick="clearAll()">
</body>
</html>
